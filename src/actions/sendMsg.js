...
const setUnread = (sender, receiver) => {
	return dispatch => {
		database
			.ref(`unreadMsgs/${sender}/${receiver}`)
			.once('value', snapshot => {
				let counter = snapshot.val() ? snapshot.val().count : 0;
...
const onlyMsg = (sender, receiver, msg = null, img = null) => {
	return dispatch => {
		const key = database
			.ref('messages/')
			.push()
...
export const sendChatMsg = (sender, receiver, msg = null, file) => {
	return dispatch => {
		if (file) {
